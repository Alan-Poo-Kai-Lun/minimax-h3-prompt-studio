(() => {
  ...
  const textRules = [
    ...
    ["#scriptWorkflow .workflow-steps span:nth-child(3)", "3 剧情检查", "3 Story check"],
    ["#scriptWorkflow .workflow-steps span:nth-child(4)", "4 人工确认", "4 Approval"],
    ["#scriptWorkflow .workflow-steps span:nth-child(5)", "5 H3 提示词", "5 H3 prompt"],
    ...
    ["#reviewScript", "检查剧情", "Check story"],
    ["#stopReview", "停止检查", "Stop review"],
    ["#storyReviewTitle", "剧情检查结果", "Story review"],
    ["#storyReviewTextLabel", "独立审查意见", "Independent review"],
    ["#revisedScriptLabel", "建议修正版（不会自动覆盖原剧本）", "Suggested revision (does not overwrite the original)"],
    ["#copyStoryReview", "复制检查结果", "Copy review"],
    ["#applyRevisedScript", "采用修正版", "Use revised script"],
  ...
  const englishChangelog = `<b>v0.8.6</b><ul><li>Fixes approved screenplay dialogue being omitted from final H3 prompts</li><li>Builds a per-segment mandatory dialogue manifest that preserves the original language, exact words, and speaker</li><li>Post-generation validation restores missing or paraphrased dialogue as H3 dialogue tags and requires visible lip synchronization</li></ul><b>v0.8.5</b><ul><li>Script workflow adds Story Check with deterministic checks for segments, timing, reference numbering, dialogue density, and repetition</li><li>An independent AI review audits causality, motivation, continuity, transitions, and ending, then returns a score, issue list, and full revision</li><li>The original script is never overwritten automatically; manual edits mark an older review as outdated</li><li>Project save and import preserve story-review results</li></ul><b>v0.8.4</b><ul><li>Hybrid no longer requires first or last frames and can directly combine images, Video, and Audio references</li><li>Switching modes retains all media; inactive media is temporarily excluded from requests</li><li>Video cards support custom-time first-frame capture plus player, duration, resolution, and size metadata</li><li>Audio cards include playback, duration, and file size</li></ul><b>v0.8.3</b><ul><li>Improved Hybrid keyframe-role detection; v0.8.4 relaxes that restriction for the actual workflow</li></ul><b>v0.8.2</b><ul><li>The @ media menu supports Picture, Video, and Audio with independent numbering</li><li>Direct prompt and Script workflow can insert and navigate Video/Audio tags</li><li>Video and audio upload zones support drag and drop, type filtering, and visual feedback</li></ul><b>v0.8.1</b><ul><li>Direct prompt and Script workflow now show independently</li><li>Hybrid supports independently numbered Video references and H3 metadata</li><li>Workflow order corrected to 01 Creative, 02 Mode, and 03 Canvas</li></ul><b>v0.8.0</b><ul><li>Complete Chinese and English interface coverage</li><li>Floating searchable managers for history, templates, and Skills</li><li>Language regression checks for static and dynamic UI</li></ul><b>v0.7.1</b><ul><li>Resizable columns with remembered widths</li><li>Focus mode for each workspace column</li><li>Improved layout at 1366px width</li></ul><b>v0.7.0</b><ul><li>Three-column desktop workspace</li><li>Independent column scrolling and responsive fallback</li></ul><b>Earlier versions</b><ul><li>Picture ordering, Skills, templates, script workflow, multiple AI backends, and standard H3 formatting</li></ul>`;
  const english087 = `<b>v0.8.7</b><ul><li>The approved screenplay is locked as the sole story source; unapproved plot events, dialogue, thought bubbles, slogans, props, and claims are forbidden</li><li>Dialogue language descriptions are normalized to H3 labels such as [Malay], [Chinese], and [English]</li><li>Model-written, duplicate, or extra dialogue is removed before exact approved lines are reinserted once in the correct segment</li><li>Malformed retention analysis is normalized with shot scope, fixed relationship markers, and explanations</li></ul>`;
  const english088 = `<b>v0.8.8</b><ul><li>Dialogue is inserted into its original timed shot, with stable speaker IDs across shots and segments</li><li>Brand narration and voiceover stay off-screen and no longer require visible lip synchronization</li><li>Retention markers and shot scopes are normalized by Picture, Video, and Audio media type</li><li>Background music remains only in non_diegetic_music instead of being duplicated in the soundscape</li><li>Conversion stops with a clear warning when source timing conflicts with segment settings</li></ul>`;
  ...
  function applyDynamic() {
    ...
    if (changelog) {
      const value = language() === "en" ? '<b>v0.8.15</b><ul><li>Automatically rebuilds a complete multi-segment result when H3 fields, segments, or Shot 1 are missing</li><li>Restores approved dialogue only after the repaired H3 structure passes validation</li><li>Keeps the raw model output visible if two repair attempts still fail</li></ul><b>v0.8.14</b><ul><li>Checks and automatically repairs untranslated prose in custom-language H3 output</li><li>Locks fields, media labels, shots, timestamps, relationship markers, and dialogue during repair</li><li>Rejects a repair if it remains mixed-language or damages the H3 contract</li></ul><b>v0.8.13</b><ul><li>Detects untranslated English prose in custom-language scripts and automatically retries the complete translation once</li><li>A second mixed-language result is rejected while the English source remains safe</li><li>H3 prompt output now has an independent custom-language option</li><li>Approving a custom screenplay automatically carries its language into H3 output settings</li></ul><b>v0.8.12</b><ul><li>Script output supports an English + custom-language bilingual mode</li><li>The English source is read-only; the custom version remains editable and drives story review and H3 conversion</li><li>Translation locks segment numbers, fields, timing, media labels, and original dialogue</li><li>Projects and full backups preserve the custom language; both versions can be copied or downloaded together</li></ul><b>v0.8.11</b><ul><li>Side-by-side screenplay comparison with change highlights, navigation, synchronized scrolling, and editable revisions</li><li>Manual segment edits, targeted rewrites and undo preserve untouched segments</li><li>Wrong segment responses, failures and cancellation cannot overwrite the original</li></ul><b>v0.8.10</b><ul><li>Templates apply to the active workflow without stale screenplay conversion</li><li>Full settings, template, Skill, parameter and layout backups with safe merge, validation and rollback</li><li>API Keys excluded by default; projects and media remain separate</li></ul><b>v0.8.9</b><ul><li>Supports single-quoted and unquoted dialogue and explicitly rejects malformed lines</li><li>Clears invented dialogue in silent segments and unifies unambiguous character aliases</li><li>Custom speaker processing is idempotent without leftover phrases or accumulating blank lines</li><li>Preserves sound effects in mixed music clauses and flags ambiguity for review</li><li>Timing checks support minutes, clock notation, and source timeline endpoints</li></ul>' + english088 + english087 + englishChangelog : chineseChangelog;
      const currentValue = language() === "en" ? '<b>v0.8.17</b><ul><li>Reverse-engineers image prompts in Chinese or English</li><li>Samples up to eight video frames locally and reconstructs MiniMax H3 or generic video prompts</li><li>Adds media preview, optional emphasis, copy, and TXT download</li></ul><b>v0.8.16</b><ul><li>Checks raw H3 output for duplicate or empty fields before normalization and repair</li><li>Updates source build instructions and both READMEs</li></ul>' + value : value;
      if (changelog.innerHTML !== currentValue) changelog.innerHTML = currentValue;
    }
